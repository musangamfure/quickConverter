...
export default function Dropzone() {
  ...
  const convert = async (): Promise<any> => {
    ...
    for (let action of tmp_actions) {
      try {
        // If the file is HEIC, convert it to JPEG using heic2any
        let convertedFile = action.file;
        if (action.from.toLowerCase() === "heic") {
          ...
          if (jpegBlob instanceof Blob) {
            ...
            convertedFile = new File(
              [jpegBlob[0]],
              `${action.file_name}.jpeg`,
              { type: "image/jpeg" }
            );
          }
        }

        // Run the FFmpeg conversion on the (possibly converted) file
        const { url, output } = await convertFile(ffmpegRef.current, {
          ...action,
          file: convertedFile, // Use the converted file if HEIC
        });
